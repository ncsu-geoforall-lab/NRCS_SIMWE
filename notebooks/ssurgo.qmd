...
m = gj.Map(use_region=True, filename="output/coweeta_ksat_r_sda.png")
...
# Point query at the Coweeta study location (lon, lat) — avoids the multi-GB
# regional grid download that mode="grid" triggers.
...
with gs.RegionManager(raster="accumulation", flags="a"):
    m = gj.Map(use_region=True)
    m.d_shade(shade="relief", color="accumulation")
    m.d_vect(map="noaa_atlas14_station", color="blue", width=2)
    m.d_legend(
        raster="accumulation",
        title="Accumulation",
        flags="s",
...
tools.r_mapcalc(
    expression=f"rainfall_excess = runoff_depth / 24.0", # TODO divide by correct time in hours based on time of concentration
)
