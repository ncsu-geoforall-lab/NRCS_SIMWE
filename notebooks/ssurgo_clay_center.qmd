...
# Convert runoff_depth (mm) to rainfall excess.
...
with gs.RegionManager(raster="accumulation", s="s-200", flags="a"):
    ...
    m.d_rast(map=max_depth, values="0.025-0.46251")
    m.d_legend(
        raster=max_depth,
        title="Deqpth [m]",
        at="8, 12, 15, 85",
...
m.add_raster_series("rx_100yr_depth_sum", values="0.025-0.46251")
m.d_legend(
    raster=max_depth,
    title="Depth [m]",
    flags="bt",
...
tools.r_sim_water(
    elevation="elevation",
    depth="no_soil_depth",
    discharge="no_soil_disch",
    walkers_output="no_soil_particles",
...
```{python}
# | label: no_soil_depth_sum_info

tools.t_info(input="no_soil_depth_sum")
```
...
no_soils_max_depth = depth_list.split(",")[-1]

with gs.RegionManager(raster="accumulation", s="s-200", flags="a"):
    m = gj.Map(use_region=True)
    m.d_shade(shade="relief", color="naip_2021_rgb@naip")
    m.d_rast(map=no_soils_max_depth, values="0.025-1.307113")
    m.d_legend(
        raster=no_soils_max_depth,
        title="Depth [m]",
        flags="btl",
        border_color="none",
        at=(5, 50, 2, 5),
...
m = gj.TimeSeriesMap(use_region=True, width=800)
m.d_rast(map="naip_2021_rgb@naip")
m.add_raster_series("no_soil_depth_sum", values="0.025-1.307113")
m.d_legend(
    raster=no_soils_max_depth,
    title="Depth [m]",
    flags="bt",
