...
window.QuartoSupport = function() {
        ...
        Array.from(container.querySelectorAll("a")).forEach((el) => {
            ...
            if (/^(http|www)/gi.test(url)) {
                el.addEventListener("click", (ev) => {
                    ...
                    const dataPreviewLink =
                        el.getAttribute("data-preview-link");

                    // if there is a local specifcation then use that
                    if (dataPreviewLink) {
                        if (dataPreviewLink === "true" ||
                        ...
                        if (previewLinks == true ||
                            (previewLinksAuto && fullscreen)) {
                            ev.preventDefault();
                            deck.showPreview(url);
                            return false;
                        }
                    ...
                    const iframe = window.location !== window.parent.location;
                    if (iframe && !window.location.search.includes(
                                      "quartoPreviewReqId=")) {
                        ev.preventDefault();
                    ...
                    // if the user has set data-preview-link to "auto" we need
                    // to handle the event (because reveal will interpret "auto"
                    // as true)
    ...
    function tweakSlideNumber(deck) {
        deck.on("slidechanged", function(ev) {
            ...
            const onDarkBackground =
                slideBackground.classList.contains('has-dark-background')
            const onLightBackground =
                slideBackground.classList.contains('has-light-background')
            toggleBackgroundTheme(slideNumberEl, onDarkBackground,
                                  onLightBackground);
        })
    }
    ...
    function addFooter(deck) {
        ...
        // or show default unless data-footer="false" for no footer on this
        // slide
        const setSlideFooter = (ev, defaultFooterDiv) => {
            const currentSlideFooter = ev.currentSlide.querySelector(".footer");
            const onDarkBackground =
                deck.getSlideBackground(ev.currentSlide)
            ...
            if (currentSlideFooter) {
                ...
                toggleBackgroundTheme(slideFooter, onDarkBackground,
                                      onLightBackground)
            }
            ...
            if (!isPrintView()) {
                // Ready even is needed so that footer customization applies on
                // first loaded slide
                deck.on('ready',
                        (ev) => {// Set footer (custom, default or none)
                                 setSlideFooter(ev, defaultFooterDiv)});
                // Any new navigated new slide will get the custom footnote
                // check
                deck.on("slidechanged", function(ev) {
    ...
    function addChalkboardButtons(deck) {
        ...
        if (chalkboard && !isPrintView()) {
            ...
            deck.on("slidechanged", function(ev) {
                ...
                const slideButtons =
                    ev.currentSlide.getAttribute("data-chalkboard-buttons");
                if (slideButtons) {
                    if (slideButtons === "true" || slideButtons === "1") {
                        buttons = true;
                    } else if (slideButtons === "false" ||
                               slideButtons === "0") {
                        buttons = false;
                    }
                }
    ...
    function fixupForPrint(deck) {
        if (isPrintView()) {
            ...
            slides.forEach(function(
                slide) { slide.removeAttribute("data-auto-animate"); });
            window.document.querySelectorAll(".hljs").forEach(function(
                el) { el.classList.remove("hljs"); });
            window.document.querySelectorAll(".hljs-ln-code")
                .forEach(function(el) { el.classList.remove("hljs-ln-code"); });
        }
    }
    ...
    function handleWhiteSpaceInColumns(deck) {
        for (const outerDiv of window.document.querySelectorAll(
                 "div.columns")) {
    ...
    class ScrollViewToggler {
        constructor(deck) {
            ...
            this.oldScrollActivationWidth =
                deck.getConfig()['scrollActivationWidth'];
        }
    ...
    return {
        ...
        toggleScrollView :
            function() { scrollViewToggler.toggleScrollViewWrapper(); }
    };
};
