var PdfExport = (function(_Reveal) {
    ...
    function getRevealJsPath() {
        ...
        var script =
            Array.from(document.querySelectorAll('link')).find(function(e) {
                return e.attributes.href &&
                       e.attributes.href.value.search(regex) >= 0;
            });
        if (!script) {
            console.error(
    ...
    function togglePdfExport() {
        ...
        url_doc.search =
            (query_doc.toString() ? '?' + query_doc.toString() : '');
    ...
    if (Reveal && Reveal.VERSION && Reveal.VERSION.length &&
        Reveal.VERSION[0] == '3') {
        // reveal 3.x
        ...
        Plugin.togglePdfExport = function() { togglePdfExport(); };
    }
